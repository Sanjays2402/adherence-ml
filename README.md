...
**Retention sweep (CLI).** The same retention TTLs enforced by `POST /v1/admin/retention-policy/sweep` can run headless from cron or systemd timers without the in-process scheduler: `adherence-ml sweep-retention --dry-run` previews candidates per table, `adherence-ml sweep-retention` performs the sweep, and per-table overrides (`--audit-days`, `--outcomes-days`, `--webhooks-days`, `--idem-days`) tune TTLs for a single run. Pass `--json` for machine-readable output (`{dry_run, tables: [{table, cutoff, candidates, deleted}], total_deleted}`) so a wrapper script can alert when `total_deleted` is unexpectedly high. The run is structured-logged as `retention_sweep` with per-table candidate and deleted counts. Note that the CLI path bypasses the API's admin-MFA and legal-hold gates, so restrict cron credentials to a dedicated operator principal and prefer the API route when those gates matter.
