...
class PolicyOut(BaseModel):
    ...
    enabled_builtins: list[str] = Field(
        ...
        description="Subset of the supported built-in patterns currently on.",
    )
    custom_patterns: list[str] = Field(
        default_factory=list,
        description="Tenant-defined regex patterns applied after built-ins.",
    )
...
class PolicyIn(BaseModel):
    enabled_builtins: list[str] = Field(
        default_factory=list,
        description=(
            "Subset of supported built-ins to enable. Unknown names are "
            "silently dropped."
        ),
        max_length=len(BUILTIN_NAMES),
    )
    custom_patterns: list[str] = Field(
        default_factory=list,
        description=(
            "Optional list of additional regex patterns. Each must compile "
            "and stay within configured size limits."
        ),
        max_length=MAX_CUSTOM_PATTERNS,
    )
    mask: str = Field(
        default=DEFAULT_MASK,
        min_length=1,
        max_length=MAX_MASK_LEN,
        description="Literal replacement string written in place of matches.",
    )
