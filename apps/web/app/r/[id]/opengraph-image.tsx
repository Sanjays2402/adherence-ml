/**
 * OG image for shared runs at /r/<id>. 1200x630 PNG via Edge ImageResponse.
 *
 * Renders the run title, kind, top miss-probability (when payload is a predict
 * result), tags, and brand chrome so Slack/Twitter/iMessage unfurl cleanly.
 *
 * Path: GET /r/<id>/opengraph-image
 *
 * Wired into <meta property="og:image"> by Next via the file convention; the
 * /r/<id> page also opts in explicitly via generateMetadata.openGraph.images.
 */
...
export default async function Image({ params }: { params: { id: string } }) {
  const rec = await getRun(params.id);
  const title = rec?.title ?? "Run not found";
