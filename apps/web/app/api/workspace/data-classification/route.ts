...
const ALLOWED = ["public", "internal", "confidential", "restricted"] as const;

const PutSchema = z.object({
  label: z.enum(ALLOWED),
  justification: z.string().max(1024).optional().nullable(),
});
...
export async function GET(req: NextRequest) {
  try {
    const data = await apiFetch(`/v1/workspace/data-classification`, {
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const data = await apiFetch(
      `/v1/workspace/data-classification${qs}`,
      {
        method: "PUT",
