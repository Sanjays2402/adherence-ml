...
// Mirrors adherence_common.sso_enforcement bounds.
const MAX_BREAK_GLASS_SUBJECTS = 5;
const MAX_SUBJECT_LEN = 128;

const PutSchema = z.object({
  require_sso: z.boolean(),
  break_glass_subjects: z
    .array(z.string().trim().min(1).max(MAX_SUBJECT_LEN))
    .max(MAX_BREAK_GLASS_SUBJECTS)
    .default([]),
});
...
export async function GET(req: NextRequest) {
  try {
    const data = await apiFetch(`/v1/workspace/sso-enforcement`, {
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const data = await apiFetch(`/v1/workspace/sso-enforcement${qs}`, {
      method: "PUT",
