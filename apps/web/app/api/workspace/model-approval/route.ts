...
const MODES = ["disabled", "audit", "enforce"] as const;

const PutSchema = z.object({ mode: z.enum(MODES) });
...
export async function GET(req: NextRequest) {
  try {
    const data = await apiFetch(`/v1/workspace/model-approval`, {
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const data = await apiFetch(`/v1/workspace/model-approval${qs}`, {
      method: "PUT",
